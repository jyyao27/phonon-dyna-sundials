...
def get_mandatory_optional(param,input_data,executable):
   if param in ['prefix','calc_mode']:
      ...
      if mand_list:
         text = text + '<nobr><i>Mandatory&nbsp;for'+prefix+\
                    '</i>'+'&nbsp;'*2
      ...
      if opt_list:
         text = text + '<nobr><i>Optional&nbsp;for'+prefix+\
                    '</i>'+'&nbsp;'*2
...
def write_parameter_info(fout,key,value,color,family_desc,input_data,executable):
   ...
   fout.write('<div class="input_param" style="--background: '+lightendarken(color,-87.0)+'; --border: '+lightendarken(color,30.0)+'">\n')
   fout.write('<div class="family_watermark" style="--color: '+lightendarken(color,10.0)+'"><i>'+family_desc.upper() +'</i></div>\n')
   ...
   if key == 'calc_mode':
      fout.write(' To see the typical input files for different calculation modes, <a href="mydoc_generate_input.html#select_button">click here</a>.')

   fout.write('<br>'*2)

   # Mandatory/Optional box
   fout.write('<button type="button" class="collapsible" style="--background: #999999; --backgr_active: #FFFFFF; --strip-color:'+lightendarken(color,10.0) +'">'+'&#9660;'*1+'&nbsp;'*10+'Mandatory/Optional'+'&nbsp;'*10+'&#9660;'*1+'</button>')
   fout.write('<div class="content" style="--strip-color: '+lightendarken(color,10.0) +'">')
...
def write_header(fout,fout_name,title):
   ...
   fout.write('---'+'\n')
   fout.write('<!-- This file was generated by the python script: '+\
...
def create_html_table(yaml_dict,input_data,html_path,fout_name,title,executable):
   ...
   for family in family_list:
      ...
      if len(family_list) > 1:
         fout.write('<tr>\n')
         fout.write('<td colspan="3"><font size="4"><center>'+input_param[family]['description']+'</center></font></td>\n')
         fout.write('</tr>\n')
         fout.write('\n')
      ...
      for key,value in input_param.items():
         if 'family' in value.keys() and value['family'] == family:
            ...
            if 'default' in value.keys():
               fout.write('<p><i>Default:</i> '+print_value(value['default'])+'</p>')
            if 'typical' in value.keys() and key != 'calc_mode':
               fout.write('<p><i>Typical:</i> '+print_value(value['typical'])+'</p>')
            if 'dimenstions' in value.keys():
               fout.write('<p><i>Dimenstions:</i> '+print_value(value['dimenstions'])+'</p>')
            if 'options' in value.keys():
               fout.write('<p><i>Options:</i> '+str(list(map(print_value,list(value['options'].keys()))))+'</p>')
            fout.write('</td>\n')
...
def create_html_file(yaml_dict,input_data,html_path,fout_name,title,executable):
   ...
   for family in family_list:
      ...
      fout.write('<h3><a href=#'+family+' style="color:Navy; text-decoration: none; border-bottom: 3px solid '+lightendarken(color,0)+';"'+'>'+input_param[family]['description']+'</a></h3>\n<p>\n')
   ...
   for family in family_list:
      ...
      if len(family_list) > 1:
         fout.write(' <span style="display:inline-block;background:'+lightendarken(color,0.0)+';width:100%"><font size="5"><a style="color:Navy"; name='+family+'>&nbsp'+input_param[family]['description']+'</a></font></span>\n<br>\n<br>\n')

      for key,value in input_param.items():
         if 'family' in value.keys() and value['family'] == family:
            write_parameter_info(fout,key,value,color,family_desc,input_data,executable)
...
def print_inputs_to_html(fout,id_prefix,calc_mode_list):
   ...
   for calc_mode in calc_mode_list:
      os.system('python generate_input.py -c '+calc_mode+' -i '+'tmp.in > /dev/null')

      fout.write('<div id="'+id_prefix+'_'+calc_mode+'" style="display:none">\n')

      fout.write('<p style="font-size:140%; text-decoration: underline">Input File:\n')
      fout.write('<button id="copy_button" style="float: right;" onclick="CopyToClipboard_from_id(\'{}\')" title="Click to copy input file to clipboard" ><i class="fa fa-copy"></i></button>\n'.format(calc_mode))
      fout.write('</p>\n')
      fout.write('<p style="font-size:80%;text-align:left; margin-top:-1%">(click on a parameter to get its description)</p>')
      fout.write('<div id="input_text_{}" style="margin-top:-1.8%">\n'.format(calc_mode))
      ...
      for line in open('tmp.in','r'):
         ...
         if '=' in line:
            ...
            param = re.sub('\(.*\)','',param)
            line = re.sub(param,lpath+param+'_end1_'+param+'_end2',line,1)
...
def print_select_button_header(fout_name, yaml_dict):
   ...
   with open('../docs/input_parameters_perturbo.yml', 'r') as stream:
   ...
   list_calc_mode = ['qe2pert']+list(param_data_perturbo['calc_mode']['options'].keys())

   # create the drop down box
   fex.write('<hr>\n')
   fex.write('<a name="select_button"></a>\n')
   fex.write('<p>To get a <em>typical</em> input file without running the script, select the calculation type here: ')
