...
subroutine save_elph_mat_wann()
   ...
   use epr_hdf5_io, only: write_elph_mat_wann_part, read_ephmat_part
   ...
   integer(HID_T) :: elph_fid, epr_fid, epr_gid
   ...
   if(ionode) then
      !open epr.h5 file
      fname = trim(prefix)//"_epr.h5"
      call hdf_open_file(epr_fid, trim(fname), status='OLD', action='WRITE')
      call hdf_create_group(epr_fid, 'eph_matrix_wannier')
      call hdf_open_group(epr_fid, "eph_matrix_wannier", epr_gid)
...
!$omp end parallel do
         ...
         !output to hdf5 file
         if(ionode) call write_elph_mat_wann_part(epr_gid, iw, jw, ia, pep)
         !
   ...
   !close hdf file
   if(ionode) then
      call hdf_close_group(epr_gid)
      !
      call hdf_close_file(epr_fid)
...
subroutine eph_bloch2wann &
      (xq, xk_all, numk, epmat, nrvec_e, rvec_set_el, nrvec_p, rvec_set_ph, epr)
   use kinds, only: dp
   ...
   type(eph_wannier), intent(inout) :: epr
   !
   integer :: ire, irp, ik
   real(dp) :: rdotk, vec(3)
   complex(dp) :: cfac
...
!$omp do schedule(guided)
   do irp = 1, epr%ws_ph%nr
      vec = rvec_set_ph(:, epr%ws_ph%rvec(irp) )
      !
      rdotk = tpi * dot_product(xq, vec)
      ! exp(-i k*Rp)
      cfac = cmplx(cos(rdotk), -sin(rdotk), kind=dp)
      !ep_hop(:,:, irp) = ep_hop(:, :, irp) + epmatw(:, :, iq) * factor
      call zaxpy(3*ws_el%nr, cfac, epmatw(1,1), 1, epr%ep_hop(1,1,irp), 1)
   enddo
!$omp end do
!
!$omp end parallel
