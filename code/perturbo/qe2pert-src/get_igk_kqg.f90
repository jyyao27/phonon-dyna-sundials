...
subroutine get_igk_kqg(npwq, igk_k, xk, xkg, igk_kg)
   ...
   if( norm2( xg ) < eps5 ) then
      ...
      i_tmp(:) = nint( xg_cryst(:) )
      do i = 1, 3
         if(abs( xg_cryst(i) - i_tmp(i) ) > eps5 ) then
   ...
   up = npwqg
   do i = lw+1, npwqg
      if(abs(current_kin - gkin(i)) > eps5) then
         up = i - 1
         exit
      endif
   enddo
   ...
   do n = 1, npwq
      ...
      if( abs(kin - current_kin) < eps5 ) then
         do i = lw, up
            if( norm2( xgg - g(:, igk_tmp(i)) ) < eps5 ) then
               igk_kg(n) = igk_tmp(i)
               exit
            endif
         enddo
