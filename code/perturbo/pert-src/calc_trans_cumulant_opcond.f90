...
subroutine calc_trans_cumulant_opcond()
   ...
   use pert_data,  only: alat, volume, epr_fid, kc_dim
   ...
   use pert_param, only: ntemper, band_min, band_max, prefix, cum_inner_emin, &
      cum_inner_emax, cum_outer_emin, cum_outer_emax, cum_de, cum_outer_np, hole, &
   ...
   integer  :: it, ib, ik, numb, numk, spf_lw, spf_up, i, j, ie, &
      jst, jend, iout, nestep, cut_lw, cut_up, cut_tmp(2)
...
!$omp parallel default(shared) private(j, ik, ib, it, i, cut_lw, cut_up, cut_tmp, ie, tmp, ene, mdfde, sptmp)
...
!$omp atomic
      ...
      do ie = 1, nestep-1
         ...
         tmp = 0.0_dp
         do i = cut_lw, cut_up
            if( (ie+i) > cut_up ) cycle  !skip if w+E exceed boundary
