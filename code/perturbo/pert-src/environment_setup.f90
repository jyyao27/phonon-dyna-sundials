...
subroutine print_acc_info()
   use openacc
   use sys_utils, only: c_to_f_string
   implicit none
   ...
   integer(c_size_t), value :: device_mem
   ...
   do dev_num = 0, num_gpus - 1
      ...
      write (*,'(5X,"ACC device ",I0,":  ",A," (driver ",A,") - ",F5.2," GiB memory")') &
            dev_num, trim(str_name), trim(str_driver), (REAL(device_mem) / 1024.0 / 1024.0 / 1024.0)
   enddo
