...
subroutine boltz_setup()
   ...
   use pert_data, only: nsym, symop, volume, epr_fid, qc_dim, kc_dim, system_2d, alat, at
   use qe_mpi_mod, only: ionode, stdout, mp_barrier, inter_pool_comm
   use boltz_grid, only: grid, init_boltz_grid, boltz_grid_generate, init_boltz_qgrid
   use pert_param, only: ntemper, temper, efermi, ftemper, hole, find_efermi, band_min, &
      band_max, boltz_de, boltz_emax, boltz_emin, spinor, boltz_kdim, boltz_qdim, doping, debug
   ...
   use phonon_dispersion, only:  init_lattice_ifc
   implicit none
   type(grid) :: kg
   type(grid) :: qg !-xt
   type(grid) :: qg_fine
   ...
   type(lattice_ifc) :: phon, phon_fine ! -xt how to initialize?
   !
   if(ntemper < 1) then
      write(stdout, '(5x, a)') &
   ...
   if( check_exist_tet(boltz_kdim, boltz_emin, boltz_emax, band_min, band_max) ) then
      ...
      call boltz_grid_generate(kg, boltz_kdim, boltz_emin, &
         boltz_emax, band_min, band_max, nsym, symop, 'k', elec)
      !
      call init_boltz_grid(kg, elec, band_min, band_max, .false.)
   endif

   !check if we need to generate q-grid -xt
   if( check_exist_qg_tet(boltz_qdim) ) then
      !load from pre-computed prefix_qg_tet.h5 file
      call init_boltz_qgrid(qg, phon)
   else
      write(stdout,'(5x,a)') '>generating q grid ...'
      !setup (for phonon dynamics): generate qgrid
      call boltz_grid_generate(qg, boltz_qdim, -1.0_dp, -1.0_dp, -1, -1, nsym, symop, 'q')
      !
      call init_boltz_qgrid(qg, phon, .false.)
   endif

   !call init_lattice_ifc(epr_fid, kc_dim, phon_fine)
   !if( check_exist_qgf_tet(boltz_kdim) ) then
   !   call init_boltz_qgrid(qg, phon, .true., .true.)
   !else
   !   write(stdout,'(5x,a)') '>generating fine q grid ...'
   !   !setup (for phonon dynamics): generate qgrid
   !   call boltz_grid_generate(qg_fine, boltz_kdim, -1.0_dp, -1.0_dp, -1, -1, nsym, symop, 'f')
   !   !
   !   call init_boltz_qgrid(qg_fine, phon_fine, .false., .true.)
   !endif 
   ...
   logical function check_exist_qg_tet(kdim) result(lexist)
      use pert_const, only: dp
      use pert_param, only: prefix
      use hdf5_utils
      implicit none
      ...
      lexist = .true.
   end function check_exist_qg_tet   

   logical function check_exist_qgf_tet(kdim) result(lexist)
      use pert_const, only: dp
      use pert_param, only: prefix
      use hdf5_utils
      implicit none
      ...
      lexist = .true.
   end function check_exist_qgf_tet   
