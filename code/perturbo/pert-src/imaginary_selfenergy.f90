...
module imaginary_selfenergy
   ...
   public :: calc_selfel_polar, calc_selfel
contains

subroutine calc_selfel_polar &
      (elph, el, ph, kpts, qset, qwt, tmpr, ef, bmin, enk, imsgm)
   implicit none
   ...
   call start_clock('calc_selfel_polar')
   !initialize 
   ...
   if(size(tmpr).ne.ntmpr .or. size(ef).ne.ntmpr .or. size(kpts,2).ne.numk &
      ...
      call errore('calc_selfel_polar','arguments dimension mismatch.',1)
   !simple array sanity check
   if(bmax > elph%nb) &
      call errore('calc_selfel_polar','bmin and imsgm mismatch',1)
   if(elph%nb .ne. el%nb .or. elph%na .ne. ph%na) &
      call errore('calc_selfel_polar','array dimension mismatch',1)
   if(.not. elph%lpol) &
      call errore('calc_selfel_polar','not a polar material',1)
   if(npool > numq) &
      call errore('calc_selfel_polar', 'too many pools (npool > #. k-points)',1)
   ...
   do iq = qst, qend
      ...
      do ik = 1, numk
         ...
         do jb = 1, el%nb
            ...
            dt1 = gauss( delta_smear, enk(i,ik) - ekq(jb) + wq(im) )
            dt2 = gauss( delta_smear, enk(i,ik) - ekq(jb) - wq(im) )
            ib = i + bmin - 1
...
!$omp atomic
               imsgm(i, im, it, ik) = imsgm(i, im, it, ik) + scat
            enddo
         enddo; enddo; enddo
      enddo
...
!$omp critical (selfel_polar_progress)
         write(stdout,'(8x, f7.2, a1)') (100.0_dp*iter)/nq_pool, '%'
!$omp end critical (selfel_polar_progress)
      endif
...
!$omp end parallel
   ...
   call stop_clock('calc_selfel_polar')
end subroutine calc_selfel_polar


subroutine calc_selfel &
      (elph, el, ph, kpts, qset, qwt, tmpr, ef, bmin, enk, imsgm, rm_pol)
   implicit none
   type(electron_wann), intent(in) :: el
   type(lattice_ifc),   intent(in) :: ph
   type(elph_mat_wann), intent(in) :: elph
   ...
   if(present(rm_pol)) then
      if(rm_pol .and. (.not. elph%lpol)) &
         call errore('calc_selfel',"polar_split='rmpol' is for polar material",1)
   endif
...
!$omp& ekq, ukq, ltable, gkq, g2, im, ib, jb, dt1, dt2, it, ph_n, el_fkq, scat, iter)
      allocate( ltable(el%nb, numb, ph%nm) )
      allocate( gkq(el%nb, el%nb, ph%nm), g2(el%nb, el%nb, ph%nm) )
!$omp do schedule(guided)
...
!$omp end parallel
   ...
   call stop_clock('calc_selfel')
end subroutine calc_selfel

end module imaginary_selfenergy
