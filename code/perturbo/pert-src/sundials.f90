...
module sundials
   ...
   real(dp), allocatable :: epcol(:,:), pecol(:,:), ph3col(:,:)
...
subroutine fill_user_data(kg, eptemp, qg)
   ...
   if (present(qg) ) then
      ...
      allocate(pecol(qnb,qnk), ph3col(qnb, qnk))
...
integer(c_int) function f(t, svec_dist, svec_dot, udata) result(retval) bind(C)
   ...
   if (qnb > 0) then
      ...
      if (divide_grid) then
         call ph3_scat_int_interp2(qgs, phdist, ph3col)
      else
...
subroutine set_integrators_wrapper(sun_method, t0, hs, retol, &
   ...
   if (trim(sun_method) .eq. 'ark') then
      call set_ark_integrators(t0, hs_c, retol_c, abstol_c_c, abstol_ph_c, mxsteps, &
         svec_dist, ctx, arkode_mem)
   elseif (trim(sun_method) .eq. 'erk') then
      call set_erk_integrators(t0, hs_c, retol_c, abstol_c_c, abstol_ph_c, mxsteps, &
         svec_dist, ctx, arkode_mem)
   elseif (trim(sun_method) .eq. 'mri') then
      call set_mri_integrators(t0, hs_c, retol_c, abstol_c_c, abstol_ph_c, mxsteps, &
         svec_dist, ctx, arkode_mem, inner_arkode_mem, inner_stepper)
   else
      call errore('sundials', 'need to select ark, erk, or mri for method_flag', 1)
   endif
   

end subroutine

subroutine set_ark_integrators(t0, hs, retol, abstol_c, abstol_ph, mxsteps, &
         svec_dist, ctx, arkode_mem)
   !!! After initialization of svec_dist, create ark steps
   use, intrinsic :: iso_c_binding
   implicit none
   real(c_double), intent(in) :: t0
   real(c_double), intent(in) :: hs
   ...
   !call create_rk4_butcher(BTf)
   !retval = FARKStepSetTables(arkode_mem, 4, 0, c_null_ptr, BTf)
   !retval = FARKStepSetFixedStep(arkode_mem, hs)
   !if ( retval /= 0) then 
   !   call errore('sundials', 'Error in set fixed step', 1)
   !endif
   ...
   retval = FARKStepSetInterpolantType(arkode_mem, ARK_INTERP_NONE)
   if ( retval /= 0) then
...
subroutine set_erk_integrators(t0, hs, retol, abstol_c, abstol_ph, mxsteps, &
         svec_dist, ctx, arkode_mem)
...
subroutine set_mri_integrators(t0, hs, retol, abstol_c, abstol_ph, mxsteps, &
         svec_dist, ctx, arkode_mem, inner_arkode_mem, inner_stepper)
   ...
   ! local variables
   type(N_Vector), pointer :: abvec, abvec_inner
   integer(c_int) :: retval
   real(c_double) :: hf
   ...
   retval = FARKStepSetInterpolantType(inner_arkode_mem, ARK_INTERP_NONE)
   if ( retval /= 0) then
      call errore('sundials', 'Error in set ark interpolant type', 1)
   endif
   ...
   ! associate user data 
   !retval = FARKStepSetUserData(inner_arkode_mem, c_null_ptr)
   !if (retval /= 0) then
   ...
   !call FARKodeButcherTable_Free(BTf)
   !retval = FARKStepSetTableNum(inner_arkode_mem, -1, ARKODE_KNOTH_WOLKE_3_3)
   !if (retval /= 0) then
   !   if (ionode) print *, retval
   !   call errore('sundials', 'Error: set fast method', 1)
   !endif
   ! set fast step size
   !hf = 0.1*hs
   !retval = FARKStepSetFixedStep(inner_arkode_mem, hf)
   !if (retval /= 0) then 
   !   call errore('sundials', 'Error in set fast step size', 1)
   !endif
   ! create MRI inner stepper
   ...
   retval = FMRIStepSetInterpolantType(arkode_mem, ARK_INTERP_NONE)
   if ( retval /= 0) then
      call errore('sundials', 'Error in set ark interpolant type', 1)
   endif
   ...
   ! set step interpolant degree 
   !retval = FMRIStepSetInterpolantDegree(arkode_mem, 4)
   !if (retval /= 0) then 
   !   call errore('sundials', 'Error in set interpolant degree', 1)
   !endif
   !diagfile = FSUNDIALSFileOpen("sundials_butcher.out", 'w+')
   !retval = FARKStepWriteButcher(inner_arkode_mem, diagfile)
   !call FSUNDIALSFileClose(diagfile)

   !call set_tol_vec(abstol_c, abstol_ph, ctx, abvec)
   !retval = FMRIStepSVtolerances(arkode_mem, retol, abvec)
   !if ( retval /= 0) then 
   !   call errore('sundials', 'Error in set tolerances', 1)
   !endif
end subroutine
