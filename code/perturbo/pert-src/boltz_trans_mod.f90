...
subroutine trans_cond_calc(kg, ene, tmpr, ef, ncol, vol, mfd, cond, tdf, calc_mag)
   implicit none
   type(grid), intent(in) :: kg
...
subroutine extract_trans_coeff(tmpr, cond, cond_seebeck, kk_coef, sys_2d, seebeck, t_cond, ncomp2, alpha)
   implicit none
   ...
   if(present(alpha)) then
      call tensor2matrix(alpha, tcs_tmp)
   else
      tcs_tmp = tmpr * cs_tmp
   endif
   ...
   s_tmp = 0.0_dp
   if( sys_2d ) then
      c_aux(1:2, 1:2) = mat_inv2(c_tmp(1:2, 1:2))
      ! seebeck = sigma^-1 * [sigma*seebeck]
      s_tmp(1:2, 1:2) = matmul(c_aux(1:2, 1:2), cs_tmp(1:2, 1:2))

      ! T * [sigma*seebeck] * seebeck or alpha * seebeck
      c_aux(1:2, 1:2) = matmul(tcs_tmp(1:2, 1:2), s_tmp(1:2, 1:2))
   else
