...
module yaml_utils
   use kinds, only: dp
   use sys_utils, only: c_to_f_string, find_proc_file_value
...
subroutine acc_info_yaml()
   ...
   do dev_num = 0, num_gpus - 1
      ...
      write(ymlout, '(3x,"OpenACC device ",i2,": " ,a," (driver ",a, ")")') &
...
subroutine output_scatter_info_yaml(npool, nq_col, collect_pools)
   ...
   if(ionode) then
      ...
      write(ymlout,'(/,3x,a,i21)') 'total number of q-points:', sum(nq_col)
      write(ymlout,'(3x,a,i18)') 'total number of (k,q) pairs:', sum(collect_pools)
