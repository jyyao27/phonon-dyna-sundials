...
subroutine solve_phonon_velocity_findiff(iq0, enk, num_bvec, neighbors, weight, bvec, velocity)
   ...
   do im = 1, nmode
      do ib = 1, num_bvec
         ...
         !omegadiff = qg%enk(im, qg%neighbors(ib,iq0)) - qg%enk(im,iq0)
         velocity(:, im) = velocity(:, im) + weight(ib) * bvec(:,ib) * omegadiff
...
!subroutine solve_phi3mat(pht,expiqr,wq1,wq2,wq3,uf1,uf2,uf3,ph3mat)
!   implicit none
!   real(dp), intent(in) :: wq1,wq2,wq3
!   complex(dp), intent(in) ::uf1(:),uf2(:),uf3(:)
!   !complex(dp), intent(in) :: expiqr(:)
!   type(expikr_set), intent(in) :: expiqr
...
!   do a1 = 1, pht%na
!      idx = idx + 1
!      nrt = pht%phit(idx)%trip_ph%nrt
!      allocate( inc_arr(nrt))
!      exps_vec = expiqr%exps_storage(1,idx)%exps(:)
!      
!      do ir = 1, nrt
!         ifct = pht%phit(idx)%ifct(:,:,:,ir)
