...
!initial q grid and energy; velocity is not computed at this point -xt
subroutine init_boltz_qgrid(qg, ph, load, finegrid)
   use boltz_utils, only: num2kpt
   use qe_mpi_mod, only: mp_split_pools, mp_sum, inter_pool_comm
   use force_constant, only: lattice_ifc
   use phonon_dispersion, only: solve_phonon_modes
   implicit none
   type(lattice_ifc), intent(in) :: ph
   type(grid), intent(inout) :: qg !qgrid
   !if false, then qg is already initialized, no need to load from files.
   logical, intent(in), optional :: load, finegrid
   !local variable
   logical :: load_grid, fine
   integer :: iq, qst, qend
   real(dp) :: xq(3), pheig(ph%nm) !vel(3,el%nb)
   ...
   !
   !calc. pheig
   allocate(qg%enk(qg%numb, qg%nk))
   !phonon velocity is not considered right now
   !qg%vnk(3, qg%numb, qg%nk))

   qg%enk = 0.0_dp;  !qg%vnk = 0.0_dp
