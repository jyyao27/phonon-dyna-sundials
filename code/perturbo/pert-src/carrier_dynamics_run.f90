...
subroutine carrier_dynamics_run()
   ...
   use pert_param, only: ntemper, temper, band_min, band_max, prefix, boltz_qdim, &
      ...
      boltz_acc_thr, boltz_nstep_min, &
      solver, nphstep, sun_method, dyna_phph, &
   ...
   if ( dyna_phph ) then
      ...
      if (divide_grid) then
         call boltz_ph3scat_setup(qg, phon, phont, divide_grid)
      else
   ...
   if(boltz_norm_dist .or. (boltz_acc_thr > drift_accel_tolerance)) then
   ...
   subroutine cdyna_setup(filename, kgrid, fid, gid, dist, &
      ...
      if(trim(boltz_init_dist) .eq. 'restart') then
         ...
         case('fermi')
            ...
            ! mid line is boltz_init_e0,efe and efh are manually set
            efe = 6.002 / ryd2ev
            efh = 6.736 / ryd2ev
            call init_dist_fermi_separate(kgrid, dist, boltz_init_e0, efe, efh, boltz_init_smear)
         case ('lorentz')
