...
subroutine init_elph_mat_wann(file_id, kdim, qdim, ep, lep_hop)
   use hdf5_utils
   use epr_hdf5_io, only: read_elph_mat_wann
   ...
   if(load_ep_hop) then
      ...
      do jw = 1, ep%nb
      do iw = 1, ep%nb
         call mp_bcast(ep%epr(iw,jw,ia)%ep_hop(:,:,:), ionode_id, inter_pool_comm)
      enddo; enddo; enddo
   endif
