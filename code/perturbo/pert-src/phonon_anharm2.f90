...
subroutine calc_selfph(ph, pht, kpts, qg, tmpr, enk, imsgm)
   ...
   !endif
   !output debug info
   write(stdout,'(5x, 4(a,i8,1x))') &
      'nk_pool', nk_pool, 'step_k:', step_k, 'nstep_k:', nstep_k, 'step_max:', step_max
...
!$omp& scat, iprocess, exp_ikr1, expiqr) 
...
!$omp do schedule(guided) 
      do iq = qst_pool, qend_pool
         ...
         do n = kst, kend
            ...
            !call start_clock('get ikr2')
            call get_exp_ikr(xkq, pht%rvect_set(:,2,:), expiqr)   
            !call stop_clock('get ikr2')
            !call start_clock('get ikr')
            expiqr = expiqr * exp_ikr1
            !call stop_clock('get ikr')
            ...
            do iprocess = 1, 3
               ...
               do jb = 1, ph%nm
                  ...
                  !call solve_phi3mat(pht, expiqr, wq1, wq2, wq3, &
