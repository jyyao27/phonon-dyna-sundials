...
SUBROUTINE ph_lr_orthoUwfc (xk, npw, igk_kk, xkq, npwq, igk_kq, &
  ...
  USE ldaU,             ONLY : Hubbard_projectors, nwfcU, & !, wfcU, copy_U_wfc
  ...
  ELSEIF (Hubbard_projectors=="norm-atomic") THEN
     ...
     !     'Gamma-only calculation for this case not implemented', 1 )
  ELSE
     !WRITE(stdout,*) "Hubbard_projectors_type =", Hubbard_projectors
     CALL errore ("hp_sphi"," This Hubbard_projectors_type is not valid",1)
  ENDIF
  !
