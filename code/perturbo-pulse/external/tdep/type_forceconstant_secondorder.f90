...
use konstanter, only: r8, i8, lo_iou, lo_tol, lo_sqtol, lo_pi, lo_twopi, lo_huge, lo_hugeint, lo_exitcode_param, lo_exitcode_symmetry, &
                      lo_freqtol, lo_kb_hartree, lo_frequency_Hartree_to_THz, lo_forceconstant_2nd_eVA_to_HartreeBohr, lo_exitcode_io
use gottochblandat, only: tochar, walltime, lo_clean_fractional_coordinates, lo_chop, lo_sqnorm, open_file, lo_unflatten_2tensor
...
subroutine readfromfile(fc, p, fn, mem, verbosity)
    !> the force constant
    class(lo_forceconstant_secondorder), intent(out) :: fc
...
subroutine set_ewald_and_enforce_borncharge_hermiticity(fc, p, mem, verbosity, fixlambda)
    !> forceconstant
    class(lo_forceconstant_secondorder), intent(inout) :: fc
    !> structure
    type(lo_crystalstructure), intent(in) :: p
