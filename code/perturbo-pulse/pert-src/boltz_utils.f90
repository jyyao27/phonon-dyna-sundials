...
!> Various functions used in boltzmann subroutines
module boltz_utils
...
!syp - [TODO] double check for the nint function, it seems that floor function is more appropriate 
...
pure function kpt2num(kpt, nk) result(num)
   ...
   num = -1
   ! basically, kpt is [-0.5,0.5), but here would like to use some 
   ! additional codes to make sure that 
   ! fold to the Gamma-centered FBZ, in crystal coordinate
   ! and check if kpt(i)*nk(i) is a integer or not.
   ! syp - [TOCHECK] @20241010, I suspect the use of nint here
   ! nint rounds to the nearest whole number
   ! nint(-0.5) = -1, nint(-0.3) = 0, nint(0.5) = 1
   ! Here the code will make -0.5 jump to +0.5, which is the instability 
   ! the developer mentioned in setup_kq_pairs about using xq = -xq 
   ! instead of xq = num2kpt(iqdx)
   ! kpt ~ [-0.5,0.5) -> xkr ~ (-nk/2, nk/2]
   xkr(:) = (kpt(:) - nint(kpt(:))) * nk(:)
   dis(:) =  xkr(:) - nint(xkr(:))
...
pure function kpts_minus(ikq, ik, nk, nq) result(iq)
   ...
   !> map a decimal number back to 3D coordinates, the counting order is z,y,x
...
!
! Given the number of bands and phonon modes, these values are packed into a
...
!$acc routine seq
   ...
   integer, intent(in), value :: nbnd
   integer :: band(3)
   ! mu <= nmodes, nk <= nbnd, mkq <= nbnd
   band(1) = ibits(idx,  0,  8) + 1
   band(2) = ibits(idx,  8,  8) + 1
   band(3) = ibits(idx, 16, 15) + 1
...
pure function band2idx(band, nbnd) result(idx)
   ...
   idx = 0
   !sypeng: unsure
   call mvbits(band(1) - 1, 0,  8, idx,  0)
   call mvbits(band(2) - 1, 0,  8, idx,  8)
   call mvbits(band(3) - 1, 0, 15, idx, 16)
...
pure function calc_adaptive_smear(vk,vkq,ndim) result(smear)
   use pert_data, only: nat,bg, alat
   implicit none
...
!   enddo
   ...
   ! just use dv  
   smear = norm2(vk(:) - vkq(:)) * norm2(bg(1,:))/ ndim / sqrt(3.0_dp) / alat
