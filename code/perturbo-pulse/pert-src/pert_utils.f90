...
module pert_utils
   ...
   public :: find_free_unit, binary_search, hmat_diag, hmat_upper_diag, match_table, match_table_all
   public :: match_table_absorp_emit_ph, match_table_absorp_emit_ph_dyna
   public :: match_table_pulse
   public :: get_exp_ikr, fermi, mfermi_deriv, abs2, gauss, bose, mbose_deriv, converged, pure_exp_ikr, converged_vec
   public :: random_uniform, random_cauchy, fermi_2nd_deriv, mat_inv3, mat_inv2
...
subroutine match_table_pulse(ekq, ek, wq, pulse, wcut, e_thr, ltable)
   implicit none
   real(dp), intent(in):: ekq(:), ek(:), wq(:), pulse, wcut, e_thr
   ...
   do m = 1, size(wq)
      ...
      if(e_thr < 0.0_dp) then
         ...
         do i = 1, size(ek)
         do j = 1, size(ekq)
            ltable(j,i,m) = abs(abs(abs(ek(i)-ekq(j)) - pulse) - wq(m)) < e_thr
         enddo; enddo
      endif
   enddo
end subroutine match_table_pulse
...
subroutine match_table_absorp_emit_ph_dyna(ekq, ek, wq, wcut, e_thr, ltable)
   implicit none
   real(dp), intent(in):: ekq(:), ek(:), wq(:), wcut, e_thr
   logical, intent(out) :: ltable(:,:,:) !ltable(size(ekq), size(ek), size(wq))
   integer :: i, j, m
   ...
   do m = 1, size(wq)
      ...
      if(e_thr < 0.0_dp) then
         ...
         do j = 1, size(ekq)
            if(ekq(j) < wcut) cycle
           !ltable(j,i,m) = abs(abs(ek(i)-ekq(j)) - wq(m)) < e_thr
            if (abs(ek(i)  + wq(m)  - ekq(j)) < e_thr .or. &
                abs(ek(i)  + ekq(j) -  wq(m)) < e_thr .or. &
                abs(ek(i)  - wq(m)  - ekq(j)) < e_thr) then
   ...
   enddo
end subroutine match_table_absorp_emit_ph_dyna

subroutine match_table_absorp_emit_ph(ekq, ek, wq, wcut, e_thr, ltable)
   implicit none
   real(dp), intent(in):: ekq(:), ek(:), wq(:), wcut, e_thr
   logical, intent(out) :: ltable(:,:,:) !ltable(size(ekq), size(ek), size(wq))
   integer :: i, j, m
   ...
   enddo
end subroutine match_table_absorp_emit_ph

subroutine match_table_all(ekq, ek, wq, wcut, e_thr, ltable)
   implicit none
   real(dp), intent(in):: ekq(:), ek(:), wq(:), wcut, e_thr
   logical, intent(out) :: ltable(:,:,:) !ltable(size(ekq), size(ek), size(wq))
   integer :: i, j, m
   ...
   if(e_thr < 0.0_dp) then
      ...
      do m = 1, size(wq)
         ...
         do i = 1, size(ek)
            ...
            do j = 1, size(ekq)
               if(ekq(j) < wcut) cycle
               ltable(j,i,m) = abs(ekq(j) - ek(i) - wq(m)) < e_thr
            enddo
         enddo
      enddo
   endif
end subroutine match_table_all
...
complex(dp) pure function pure_exp_ikr(xpt, vec) result(expikr)
   implicit none
   real(dp), intent(in) ::xpt(3), vec(3)
   real(dp) :: rdotk

   !syp - becasue x*k = 1 not 2pi due to Perturbo convention
   !    - see pert_data.f90 for details
   rdotk = dot_product(xpt, vec) * twopi
   expikr = dcmplx(cos(rdotk), sin(rdotk))
end function pure_exp_ikr
...
! compute -dn/dE
real(dp) elemental function mbose_deriv(kt, de) result(df)
   implicit none
   real(dp), intent(in) :: kt, de  ! kt: k*T; 

   ! syp - the minus sign due to we calculate -dn/dE
   !       it's different with mfermi_deriv where there is no minus sign
   df = - bose(kt, de) * bose(kt, -de) * (1.0_dp / kt)

end function mbose_deriv
...
#endif
   implicit none
   real(dp), intent(in), value :: kt, wq  ! need to make sure wq > 0
...
logical pure function converged_vec(prev, current, thr)
   implicit none
   real(dp), intent(in) :: prev(:), current(:), thr
   integer :: i
   real(dp) :: dist(size(prev)), diff(size(prev))
   
   dist = 0.0_dp;  converged_vec = .false.
   do i = 1, size(prev)
      dist(i) = (current(i)-prev(i))**2
      diff(i) = sqrt(dist(i)) / sqrt(prev(i)*prev(i))
   enddo

   !if all elements are converged, then converged = .true.
   if(all(diff < thr)) converged_vec = .true.

   !if(diff < thr) converged = .true.
end function converged_vec
...
!Performs a direct calculation of the inverse of a 3×3 matrix.
pure function mat_inv2(A) result(B)
