...
subroutine carrier_dynamics_postproc()
   ...
   use hdf5_utils
   use boltz_trans_mod, only: trans_dos_calc
   use boltz_trans_output, only : output_dos
   implicit none
   ...
   if( dyna_phph ) then
      ...
      nestep_ph = int( floor((emax_ph-emin_ph)/boltz_de_ph) ) + 5
   ...
   !open files
   allocate( dos_tmp(nestep_ph))
   
   if (dyna_phph) then
      call trans_dos_calc(qg, ene_ph, dos_tmp)
      if(ionode) call output_dos(ene_ph, dos_tmp)
   endif
