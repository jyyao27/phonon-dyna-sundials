...
#error At least one of SCAT_FWD or SCAT_REV must be defined.
         ...
         bands = idx2band( scatter_channels(ns)%bands_index, kg%numb )
         m = bands(1);  n = bands(2);  im = bands(3)
         !write(119,'(a20,1x,7(I5,1x))')'ik,ikq,iq,inq,m,n,im',ik,ikq,iq,inq,m,n,im
...
subroutine check_colinear_q(q1, q2, col)
   implicit none
   real(dp), intent(in) :: q1(3)
   real(dp), intent(in) :: q2(3)
   logical, intent(out) :: col
   real(dp) :: dot, nq1, nq2, angle
   dot = dot_product(q1, q2) 
   nq1 = NORM2(q1)
   nq2 = NORM2(q2)
   col = .false.
   if (nq1 > 1.0E-6_dp .and. nq2 > 1.0E-6_dp) then
      angle = dot / (nq2*nq1)
      if ((abs(angle - 1.0_dp) < 1.0E-2_dp) .or. &
         (abs(angle + 1.0_dp ) < 1.0E-2_dp)) then
         col = .true.
      endif
   endif
end subroutine check_colinear_q
...
!$omp& wq0, wq1, wq2, g2, e_delta, ocq0, ocq1, ocq2, dscat, occ_sum_0, occ_sum_1, occ_sum_2, &
!$omp& scat_index, &
!$omp& iq1c, ipt1c, ipt1, xinc, yinc, zinc, pos_vec, col, q0, q1, pop_shift) &
!$omp& reduction(+:ph3col_tmp)
   do is = 1, ph3_num_scat
      ! key step, to keep g2 the same 
      g2 = tscat_ph(is)%g2
      ...
      do zinc = -1, 1 
         ...
         ! check colinearity
         q0 = num2kpt( iq0 - 1, qg%ndim)
         q1 = num2kpt( iq1 - 1, qg%ndim)
         call check_colinear_q(q1, q0, col)
         ...
         if (abs(wq0 + wq1 - wq2) < e_thr)then  
            ...
            if (col) then
               pop_shift = norm2(q1) * wq0 - norm2(q0) * wq1
               if (abs(pop_shift)/delta_smear_ph > 1.0E-5_dp) then
                  dscat = dscat * (1.0_dp + (wq2-wq0-wq1) * norm2(q1) / pop_shift)
               endif
            endif
         ...
         elseif (abs(wq0 - wq1 + wq2) < e_thr) then  
            ...
            if (col) then
               pop_shift = norm2(q0+q1) * wq0 + norm2(q0) * wq2
               if (abs(pop_shift)/delta_smear_ph > 1.0E-5_dp) then
                  dscat = dscat * (1.0_dp + (wq1-wq0-wq2) * norm2(q0+q1) / pop_shift)
               endif
            endif
