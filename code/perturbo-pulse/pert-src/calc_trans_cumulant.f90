...
subroutine calc_trans_cumulant()
   ...
   use pert_data,  only: volume, epr_fid, kc_dim, alat, system_2d
   ...
   integer  :: it, ib, ik, numb, numk, spf_lw, spf_up, i, j, iter, step, nstep, &
      jst, jend, iout, nestep, cut_lw, cut_up, cut_tmp(2), ie, nn, icount, ncount
...
!   endif
   ...
   icount = 0
!$omp parallel default(shared) private(j, ik, ib, it, i, cut_lw, cut_up, cut_tmp, &
!$omp&  tmp, ie, tmp_cs, tmp_kk, ene, mdfde, sptmp, cumsum, iter)
...
!$omp do schedule(guided)
   do j = jst, jend
      ...
      cut_tmp = spectral_cut_tail(sptmp, spf_lw, dw, hole, weight_cut)
      cut_lw = cut_tmp(1)
      cut_up = cut_tmp(2)
...
!   endif
   ...
   do it = 1, ntemper
      ...
      call extract_trans_coeff &
         (temper(it), cond(:,it), cond_seebeck(:,it), kk_coef(:,it), system_2d, seebeck(:,it), therm_cond(:,it), 6)
   enddo
   call output_trans_coef(temper, efermi=efermi, dens=dens, cond=cond, seebeck=seebeck, t_cond=therm_cond)
