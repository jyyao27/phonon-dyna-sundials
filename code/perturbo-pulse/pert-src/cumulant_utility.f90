...
subroutine init_cum_wgrid &
   ...
   if(np < 0) call errore('init_cum_wgrid','negative np',1)
   if(present(npf)) then
      if(npf < 0) call errore('init_cum_wgrid','negative npf',1)
   endif
...
pure function spectral_cut_tail(specfunc, lb_spf, dw, hole, weight_cut) result(boundary_cut)
   implicit none
   integer,  intent(in) :: lb_spf
   logical,  intent(in) :: hole
   real(dp), intent(in) :: specfunc(lb_spf:), weight_cut, dw
   ! new index range
   integer :: boundary_cut(2)
   !local
   real(dp) :: weight
   integer  :: ub_spf, i
