...
subroutine boltz_setup()
   ...
   ! check if qgrid tet file already exists
   logical function check_exist_qg_tet(kdim) result(lexist)
      use pert_const, only: dp
      use pert_param, only: prefix
      use hdf5_utils
      implicit none
      integer, intent(in) :: kdim(3)   ! qgrid dimension
      logical :: has_file              ! flag to indicate if file exists
      character(len=120) :: fname      ! tet file name
      integer(HID_T) :: file_id        ! file ID
      integer :: itmp(3)               ! grid dimension read from file
      !real(dp) :: erange(2)
