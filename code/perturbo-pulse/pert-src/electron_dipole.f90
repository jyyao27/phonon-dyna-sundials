...
subroutine electron_dipole()
   ...
   use pert_param, only: fklist, fqlist, ntemper, temper, efermi, band_min, &
      ...
      cond_bmin, pulse, prefix
   use transition_dipole, only: calc_trans_dipole
   !
   ...
   allocate(dipole(3, 3, numb, kl%nvec), enk(numb, kl%nvec))
   
   call calc_trans_dipole(elph, elec, phon, kl%vec, ql%vec, ql%weight, &
         temper(1), efermi, cond_bmin, band_min, pulse, enk, dipole)
   !output results
   if(ionode) then
      call hdf_open_file(fid, trim(prefix)//"_dipole.h5", status='NEW')
      call hdf_write_dataset(fid, 'enk', enk)
      call hdf_write_dataset(fid, 'dipole_r', real(dipole))
      call hdf_write_dataset(fid, 'dipole_i', aimag(dipole))
      call hdf_close_file(fid)
   endif
   !   call output_imsigma(.true. , temper, efermi, enk, imsigma, 'el')
   !   call output_imsigma(.false., temper, efermi, enk, imsigma, 'el')
   !      call output_imsigma_yaml(temper, efermi, enk, imsigma, kl%nvec, kl%vec)
   !
   !endif
   !release memory
   deallocate(enk, dipole)
   
   if(ionode) write(stdout,'(5x, a)') '>dipole done.'
end subroutine electron_dipole
