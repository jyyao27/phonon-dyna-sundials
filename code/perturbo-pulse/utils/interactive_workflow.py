...
#
workflow_filename = '../docs/files_description.yml'
...
#
workflow_filename = '../docs/workflow.yml'
...
with open(graph_filename,'r') as f:
   ...
   for i,line in enumerate(lines):
      ...
      if '<!--' in line and line.split()[1].replace('&#45;','-') in interactive_nodes:
...
html.write('<!-- For maphighlight redefine post-content img, otherwise, the map boxes would be shifted down-->\n')
html.write('<style>\n')
...
#
html.write('<p>To get the calculation mode requirements, outputs, and the input file, click on a calculation mode name in the graph below or select the calculation mode from the list (recommended for mobile devices): \n\n')
html.write('<select id="CalcModeSelect" onchange="ShowBlockInteractive(this.options[this.selectedIndex].value)">\n')
...
html.write('<br>\n'*2)
html.write('<img src="images/graph.svg" class="map" style="width:{}px;height:{}px" alt="Workflow" usemap="#workflow" />\n\n'.format(image_width,image_height))
...
for node in interactive_nodes:
   ...
   html.write('<p style="font-size:30px; text-align:center;"> Calculation mode: <code>{0}</code></p>\n\n'.format(node))
   html.write('</div>\n\n')
   ...
   computes = re.sub(r'\$\$(.+?)\$\$',r'<script type="math/tex">\1</script>',computes)

   computes = computes + tutorial_link

   html.write('<!--{}-->\n'.format(node))
   html.write('<div id="hide_computes_block_{}" markdown="span" class="alert alert-success" style="display:none" role="alert"><i class="fa fa-server fa"></i> <b> Computes:&nbsp;</b>'.format(node))
   html.write(computes)
   html.write('</div>\n\n')
...
for node in interactive_nodes:
   ...
   html.write('<div id="hide_req_img_out_block_{}" class="req_img_out_wrapper" style="display:none">\n\n'.format(node))
   ...
   html.write('    <p style="font-size:140%; text-decoration: underline">Required Files:</p>\n')
   html.write('    <p style="font-size:80%;text-align:center; margin-top:4%">(click on a filename to get its description)</p>\n')
   html.write('    <ul style="list-style-type:none;padding-left: 0">\n'.format(node)) # Start list
   ...
   for i,file_req in enumerate(file_req_list):
      html.write('    <li><div style="cursor:pointer" onclick="ShowFileDesc(this.id,\'left\')" id="{0}">\n'.format(file_req))
      html.write('    <img src=images/icons/{}.svg style="width:8%" >\n'.format(files_desc_dict[file_req]['type']))
      html.write('&nbsp;&nbsp;{0:<30}\n'.format(file_req))
      html.write('    </div></li>\n')
   ...
   for i,file_opt in enumerate(file_opt_list):
      html.write('    <li><div style="cursor:pointer" onclick="ShowFileDesc(this.id,\'left\')" id="{0}">\n'.format(file_opt))
      html.write('    <img src=images/icons/{}.svg style="width:8%" >\n'.format(files_desc_dict[file_opt]['type']))
   ...
   html.write('     <img src="images/PERTURBO_hourglass_arrows.svg" alt="PERTURBO" style="display:block; width:100%; margin-left:auto; margin-right:auto" />\n')
   html.write('  </div>\n\n') # End image block
   ...
   html.write('    <p style="font-size:140%; text-decoration: underline">Output Files:</p>\n')
   html.write('    <p style="font-size:80%;text-align:center; margin-top:4%">(click on a filename to get its description)</p>\n')
   html.write('    <ul style="list-style-type:none;padding-left: 0">\n'.format(node)) # Start list
   
   for i,file_out in enumerate(workflow_dict[node]['outputs']):
      
      html.write('    <li><div style="cursor:pointer" onclick="ShowFileDesc(this.id,\'right\')" id="{0}">\n'.format(file_out))
      html.write('    <img src=images/icons/{}.svg style="width:8%" >\n'.format(files_desc_dict[file_out]['type']))

      if 'deprecated' in files_desc_dict[file_out].keys() and files_desc_dict[file_out]['deprecated']:
         html.write('&nbsp;&nbsp;<p style="display:inline;color:#5e5d5d">{0:<30} (deprecated)</p>\n'.format(file_out))
...
html.write('<div id="hide_file_desc_block" class="req_img_out_wrapper" style="display:none;margin-top:0%">\n\n'.format(node)) # Start hide_file_desc_block
...
for file_req in files_desc_dict.keys():
   ...
   html.write('  <div class="left_require_file" id="hide_descblock_left_{}" style="display:none">\n'.format(file_req)) # Start desc block

   html.write('  <div style="display:flex; align-items: center;">') # Start align block

   html.write('  <img src=images/icons/{}.svg style="width:10%" >\n'.format(files_desc_dict[file_req]['type']))
   ...
   if 'obtained from' in files_desc_dict[file_req].keys():
      ...
      if calc_mode_from in interactive_nodes:
         html.write('    <p><i>Obtained from: </i> <code style="cursor:pointer" onclick="ShowBlockInteractive(\'{0}\')">{0}</b></code>\n'.format(files_desc_dict[file_req]['obtained from']))
      else:
         html.write('    <p><i>Obtained from: </i> <code>{0}</b></code>\n'.format(files_desc_dict[file_req]['obtained from']))
   ...
   if 'format example' in files_desc_dict[file_req].keys():
      ...
      if 'deprecated' in files_desc_dict[file_req].keys() and files_desc_dict[file_req]['deprecated']:
         html_part, name_part = example_link.split('#')

         example_link = f'{html_part}?expand=true#{name_part}'

      html.write('<a href="{}" target="_blank">File format example.</a>'.format(example_link))
   html.write('    </div>\n')
...
for file_req in files_desc_dict.keys():
   ...
   html.write('  <div class="right_outputs_file" id="hide_descblock_right_{}" style="display:none">\n'.format(file_req)) # Start desc block
